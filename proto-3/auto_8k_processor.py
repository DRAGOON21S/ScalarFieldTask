...
# Import the 8-K processor from the 8-k_jsonner module
import importlib.util

def import_financial_processor():
    """Import the FinancialDataProcessor from 8-k_jsonner.py"""
    try:
        spec = importlib.util.spec_from_file_location("jsonner", "8-k_jsonner.py")
        jsonner_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(jsonner_module)
        return jsonner_module.FinancialDataProcessor
    except Exception as e:
        print(f"Error importing FinancialDataProcessor: {e}")
        sys.exit(1)

FinancialDataProcessor = import_financial_processor()
...
class Auto8KProcessor:
    def __init__(self, api_key: str, base_directory: str = None, max_workers: int = 3):
        ...
        self.processor = FinancialDataProcessor(api_key)
...
def main():
    ...
    parser.add_argument('-w', '--workers', type=int, default=3,
                       help='Maximum number of parallel processing threads (default: 3)')
    ...
    try:
        ...
        if args.dry_run:
            ...
            for company, year, input_path, output_path in files:
                ...
                print(f"  {company}/{year}: {os.path.basename(input_path)}{skip_reason}")
            return
...
python auto_8k_processor.py

# Process only Apple Inc files
python auto_8k_processor.py -c "Apple_Inc"

# Process only 2021 files
python auto_8k_processor.py -y "2021"

# Force reprocess all files (even if already processed)
python auto_8k_processor.py -f

# Use more workers for faster processing
python auto_8k_processor.py -w 5

# Dry run to see what would be processed
python auto_8k_processor.py --dry-run

# Process specific company and year with custom API key
python auto_8k_processor.py -c "Apple_Inc" -y "2021" -k "your_api_key"
"""
