...
class Auto10KProcessor:
    def __init__(self, api_key: str, base_directory: str = None, max_workers: int = 2):
        """
        Initialize the automatic 10-K processor
        ...
        # Import the processor from 10-K-JSONNER.py
    ...
    def _import_processor(self):
        """Import TenKProcessor from 10-K-JSONNER.py"""
        try:
            # Get the path to 10-K-JSONNER.py
            jsonner_path = self.base_directory / "10-K-JSONNER.py"
            if not jsonner_path.exists():
                raise FileNotFoundError(f"10-K-JSONNER.py not found at {jsonner_path}")
            
            # Import the module
            spec = importlib.util.spec_from_file_location("tenk_jsonner", str(jsonner_path))
            ...
            return jsonner_module.TenKProcessor(api_key=self.api_key)
        except Exception as e:
            self.logger.error(f"Failed to import TenKProcessor: {e}")
            raise
    ...
    def find_all_10k_files(self) -> List[Tuple[str, str, str, str]]:
        """
        Find all 10-K files in the directory structure
        ...
        self.logger.info(f"Scanning for 10-K files in: {self.companies_dir}")
        ...
        for company_dir in self.companies_dir.iterdir():
            ...
            output_base = company_dir / "output_10k"
            ...
            for year_dir in company_dir.iterdir():
                ...
                year = year_dir.name
                ten_k_dir = year_dir / "10-K"
                
                if not ten_k_dir.exists():
                    self.logger.debug(f"No 10-K directory found for {company_name}/{year}")
                    continue
                ...
                # Find all .txt files in the 10-K directory
                for file_path in ten_k_dir.glob("*.txt"):
        ...
        self.logger.info(f"Found {len(files_found)} 10-K files to process")
        return files_found
    ...
    def process_single_file(self, company_name: str, year: str, input_path: str, output_path: str) -> Dict:
        ...
        Args:
            ...
            input_path: Path to the input 10-K text file
        ...
        try:
            ...
            with self.file_lock:
                ...
                self.processor.process_tenk_document(input_file_path=input_path, output_file_path=output_path)
    ...
    def process_all_files(self, force_reprocess: bool = False, filter_company: str = None, filter_year: str = None):
        """
        Process all 10-K files found in the directory structure
        ...
        # Find all 10-K files
        files_to_process = self.find_all_10k_files()
        
        if not files_to_process:
            self.logger.warning("No 10-K files found to process")
            return
    ...
    def generate_processing_report(self, results: List[Dict]):
        ...
        report_file = f"10k_processing_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        report_data = {
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Automatically process all 10-K files in directory structure')
    ...
    parser.add_argument('-w', '--workers', type=int, default=1,
                       help='Maximum number of parallel processing threads (default: 1 - 10K files are large)')
    ...
    try:
        ...
        if args.dry_run:
            ...
            files = processor.find_all_10k_files()
...
python auto_10k.py -c "Apple_Inc"

# Process only 2021 files
python auto_10k.py -y "2021"

# Force reprocess all files (even if already processed)
python auto_10k.py -f

# Use single worker (recommended for 10-K files due to size)
python auto_10k.py -w 1

# Dry run to see what would be processed
python auto_10k.py --dry-run

# Process specific company and year with custom API key
python auto_10k.py -c "Apple_Inc" -y "2021" -k "your_api_key"

# Process Apple files with forced reprocessing
python auto_10k.py -c "Apple_Inc" -f

IMPORTANT NOTES:
- 10-K files are much larger than 8-K files, so processing takes longer
- Default worker count is 1 to avoid overwhelming the API
- Each 10-K can take several minutes to process
- Monitor your API usage and rate limits carefully
"""
