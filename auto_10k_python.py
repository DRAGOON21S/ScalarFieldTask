...
Automatically finds all 10-K filings in companies directories and processes them through the parser pipeline
"""
...
# Add the current directory to the path so we can import the FilingParserPipeline
sys.path.append(str(Path(__file__).parent))

# Try to import the FilingParserPipeline from the existing script
try:
    from filing_parser_pipeline import FilingParserPipeline
except ImportError:
    print("❌ Error: Could not import FilingParserPipeline. Make sure filing_parser_pipeline.py is in the same directory.")
    sys.exit(1)


class Auto10KProcessor:
    """
    Automated processor for finding and parsing all 10-K filings in companies directories
    """
    
    def __init__(self, companies_root_dir: str = "companies"):
    ...
    def setup_logging(self):
        ...
        log_filename = f"10k_processing_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        logging.basicConfig(
            level=logging.INFO,
        ...
        self.logger.info(f"Starting Auto 10-K Processing - Log file: {log_filename}")
    ...
    def extract_company_info(self, file_path: Path) -> Dict[str, str]:
        ...
        # Find year (should be in the path after company)
        year = "Unknown"
        for part in parts[company_idx + 1:]:
            if part.isdigit() and len(part) == 4 and part.startswith('20'):
                year = part
                break
        
        return {"company": company_name, "year": year}
    
    def find_all_10k_files(self) -> List[Path]:
        """Find all 10-K filing text files in the companies directory"""
        self.logger.info(f"🔍 Searching for 10-K files in: {self.companies_root}")
        
        if not self.companies_root.exists():
            self.logger.error(f"Companies directory not found: {self.companies_root}")
            return []
        
        # Find all .txt files in 10-K subdirectories
        filing_files = []
        ...
        for company_dir in self.companies_root.iterdir():
            ...
            # Look for 10-K directories at any level under the company
            for ten_k_dir in company_dir.rglob("10-K"):
                if ten_k_dir.is_dir():
                    self.logger.info(f"    📂 Found 10-K directory: {ten_k_dir}")
                    
                    # Find all .txt files in this 10-K directory
                    txt_files = list(ten_k_dir.glob("*.txt"))
                    for txt_file in txt_files:
                        filing_files.append(txt_file)
                        self.logger.info(f"      📄 Found filing: {txt_file.name}")
        
        self.logger.info(f"✅ Found {len(filing_files)} 10-K filing files total")
        return filing_files
    
    def create_output_directory(self, company_name: str, year: str) -> Path:
        """Create output directory for parsed results"""
        company_dir = self.companies_root / company_name
        output_dir = company_dir / "python_output_10k" / year
        output_dir.mkdir(parents=True, exist_ok=True)
        return output_dir
    
    def process_single_filing(self, filing_path: Path) -> bool:
        """Process a single 10-K filing through the parser pipeline"""
        try:
    ...
    def process_all_filings(self, max_files: int = None) -> Dict[str, Any]:
        """Process all found 10-K filings"""
        filing_files = self.find_all_10k_files()
        
        if not filing_files:
            self.logger.warning("No 10-K filing files found!")
        ...
        self.logger.info(f"🚀 Starting to process {total_files} 10-K filing files")
        self.logger.info("=" * 80)
    ...
    def save_processing_report(self, summary: Dict[str, Any]) -> str:
        ...
        report_filename = f"10k_processing_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def print_summary(self, summary: Dict[str, Any]):
        ...
        print("\n" + "=" * 80)
        print("🎯 AUTO 10-K PROCESSING SUMMARY")
        print("=" * 80)
        ...
        if self.processed_files:
            ...
            for file_info in self.processed_files:
                ...
                if company not in company_stats:
                    company_stats[company] = {"processed": 0, "years": set()}
                company_stats[company]["processed"] += 1
                company_stats[company]["years"].add(file_info["year"])
            
            for company, stats in sorted(company_stats.items()):
                years_str = ", ".join(sorted(stats["years"]))
                print(f"  🏢 {company}: {stats['processed']} files ({years_str})")
        
        # Show failed files if any
        if self.failed_files:
            print("\n❌ FAILED FILES:")
            for file_info in self.failed_files:
                print(f"  • {file_info['input_file']}: {file_info['error']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Automatically find and process all 10-K filings in companies directories',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python auto_10k_python.py
  python auto_10k_python.py --companies-dir "C:\\path\\to\\companies"
  python auto_10k_python.py --max-files 5
  python auto_10k_python.py --companies-dir companies --max-files 10
        """
    )
    ...
    try:
        # Create processor
        processor = Auto10KProcessor(args.companies_dir)
        
        if args.dry_run:
            # Just find files and show what would be processed
            filing_files = processor.find_all_10k_files()
            ...
            for i, file_path in enumerate(filing_files, 1):
                ...
                print(f"  {i:2d}. {info['company']} ({info['year']}) - {file_path.name}")
            return 0
